...
$$\text{측정 필요: } \{q, \dot{q}\} \text{ (관절 위치/속도)} + \{f_e, \tau_e\} \text{ (접촉력/토크)}$$
...
$$M(q)\ddot{q} + n(q, \dot{q}) + J^T(q) f_e = \tau \tag{1}$$
...
비선형 항의 정의:
$$n(q,\dot{q}) \triangleq b(q,\dot{q}) + g(q) + d$$
...
$$\tau = \underbrace{\tau_t}_{\text{선형화 입력}} + \underbrace{n(q,\dot{q})}_{\text{비선형 보상}} + \underbrace{J^T(q) f_e}_{\text{접촉력 보상}} \tag{4}$$

식 (1)에 대입하면:

$$\boxed{M(q)\ddot{q} = \tau_t} \tag{5}$$
...
$$M\ddot{x} = \tau_t, \quad M=2\,\text{kg},\; K_e=1000\,\text{N/m},\; x_e=0.1\,\text{m},\; x=0.08\,\text{m},\; \dot{x}=0$$
...
$$x = 0.08\,\text{m} < x_e = 0.1\,\text{m} \implies \text{접촉 없음}$$

$$\therefore f_e = 0\,\text{N}$$
...
$$\therefore x_d = 0.12\,\text{m}$$

---

**(c) 현재 상태에서 제어 입력 토크 $\tau_t$?**

$$\tau_t = K_f(f_d - f_e) = 5 \times (20 - 0) = 100$$

$$\therefore \tau_t = 100\,\text{N}$$
...
```
τ_t↑ → ẍ↑ → x↑ → f_e = K_e(x-x_e)↑ → 더 큰 τ_t 요구 → ...
```
...
| 구성 | 장점 | 단점 |
|------|------|------|
| **고 감속비 감속기** | 토크↑, 비선형성↓ (토크 리플 < 10%) | 역구동성(backdrivability)↓ → 마찰 보상 필요 |
| **저 감속비 감속기** | 역구동성↑ | 토크↓, 비선형성↑ |
| **직구동(Direct Drive)** | 역구동성 최대, 투명도(transparency) 최대 | 모터 크기↑, 비선형성↑ |
...
$$\tau_{\text{output}} = N \cdot \tau_{\text{motor}}, \quad \text{but friction} \propto N^2$$
...
$$\tau_t = G_P \Delta q = G_P J^{-1}(q) \Delta x = G_P J^{-1}(q)(x_t - x)$$
...
토크 제어 매니퓰레이터에 대한 직접 힘 제어기:

**Step 1:** 힘 제어기 출력 (선형화 공간에서):

$$\tau_t = J^T(q) \, G_F(f_d - f_e) \tag{6}$$

**Step 2:** 실제 인가 토크 (중력/동역학 보상 포함):

$$\tau = \tau_t + n(q,\dot{q}) + J^T(q) f_e \tag{7}$$

식 (6)을 (7)에 대입:

$$\tau = J^T(q) G_F(f_d - f_e) + n(q,\dot{q}) + J^T(q) f_e \tag{8}$$

**Step 3:** 댐핑 항 추가 (임의적인 가속 운동 제한):

$$\boxed{\tau = J^T(q) G_F(f_d - f_e) + n(q,\dot{q}) + J^T(q) f_e - J^T(q) K_v \dot{x}} \tag{9}$$
...
```
f_d ──(+)──→ [Force Controller Gf] ──→ J^T ──→ (τ_t) ──→ (+) ──→ τ ──→ [Manipulator] ──→ x, f_e
      (-)↑                                                    ↑ n(q,q̇) + J^T·f_e − J^T·Kv·ẋ
      f_e ←──────────────────────────────────────────────────┘
```
...
$$G_F(s) = K_P + \frac{K_i}{s} \tag{10}$$

$$U_f = K_P e_f + K_i \int e_f \, dt, \quad e_f = f_d - f_e$$
...
$$x_d - x_e = \frac{f_d}{K_e}$$

$$\boxed{x_d = x_e + \frac{f_d}{K_e}} \tag{12}$$
...
$$x_f = G_f(f_d - f_e) \tag{14}$$

- $x_e$: 환경 표면 위치 (피드포워드)
- $x_f$: 힘 제어기가 생성하는 위치 보정량
- $G_f$: 힘-to-위치 변환 제어기

> $f_d = f_e = 0$, $x_d = x_e$ 이면 → 순수 위치 제어와 동일
...
$$= G_P J^{-1}(q)\{x_e + G_F(f_d - f_e) - x\}$$

$$= \underbrace{G_P J^{-1}(q)(x_e - x)}_{\text{위치 제어 항}} + \underbrace{G_P J^{-1}(q) G_F(f_d - f_e)}_{\text{힘 제어 항}} \tag{15}$$
...
> **결론:**  
> - $G_P \uparrow$ → 위치 정밀도↑, 힘 루프 이득↑ → 잠재적 불안정  
> - 힘 제어기 $G_F$는 이 이중 구조를 고려하여 **보수적으로(conservatively)** 설계해야 한다  
...
목표: 입력 힘 $\tau$를 결정하여 로봇을 원하는 위치 $x_d$로 이동시키고, 접촉력 $f_e$를 제어한다.

#### 접촉력 모델

$$f = K_e(x - x_e), \quad x > x_e \tag{16}$$

#### 운동 방정식 (중력, 마찰 무시)

$$\tau = m\ddot{x} + K_e(x - x_e) \tag{17}$$

### 9.2 PD 제어기 설계

$$\tau = -K_v \dot{x} + K_p(x_d - x) \tag{18}$$

- $K_p$: 비례 게인 (위치 강성 역할)
- $K_v$: 속도 게인 (댐핑 역할)

식 (17), (18)을 결합하면 **폐루프 동역학**:

$$m\ddot{x} + K_v \dot{x} + (K_p + K_e)x = K_p x_d + K_e x_e \tag{19}$$

### 9.3 라플라스 해석

식 (19)의 라플라스 변환:

$$x(s) = \frac{K_p x_d(s) + K_e x_e(s)}{ms^2 + K_v s + (K_p + K_e)} \tag{20}$$

#### 폐루프 전달함수

$$H(s) = \frac{1}{ms^2 + K_v s + (K_p + K_e)} \tag{21}$$

**안정성 분석:**  
모든 계수 $m, K_v, K_p, K_e > 0$ 이므로, $H(s)$의 극점은 모두 **좌반 $s$-평면**에 존재한다.

$$\therefore \text{폐루프 시스템은 항상 안정}$$

#### 고유 진동수와 감쇠비

$$\omega_n = \sqrt{\frac{K_p + K_e}{m}}, \quad \zeta = \frac{K_v}{2\sqrt{m(K_p + K_e)}}$$

> $K_e$가 크면 $\omega_n$이 증가 → 시스템이 빠르게 진동 → 진동 억제를 위해 $K_v$ 증가 필요

### 9.4 정상 상태 분석 (Final Value Theorem)

$x_d$, $x_e$가 상수인 경우, $s \to 0$ 극한:

#### 정상 상태 위치

$$x_{ss} = \lim_{s \to 0} s \cdot x(s) = \frac{K_p x_d + K_e x_e}{K_p + K_e} \tag{22}$$

#### 정상 상태 힘

$$f_{ss} = K_e(x_{ss} - x_e) = \frac{K_p K_e (x_d - x_e)}{K_p + K_e} \tag{23}$$

#### $K_e \gg K_p$ 일 때의 근사

딱딱한 환경($K_e \gg K_p$)에서:

$$\boxed{f_{ss} \cong K_p(x_d - x_e)} \tag{24}$$
...
> 힘이 **위치 오차로부터 간접적으로** 생성되는 것이다. → 이것이 "Indirect Force Control"
...
$$\tau = M(q)\ddot{q} + n(q,\dot{q}) + \tau_e$$

$$\tau = M(q)\ddot{q} + n(q,\dot{q}) + J^T(q) f_e \tag{25}$$

#### 환경 모델

$$f_e = K_e(x - x_e), \quad K_e \in \mathbb{R}^{n \times n}, \quad x_e \in \mathbb{R}^{n \times 1} \tag{26}$$

### 10.2 다차원 강성 제어기 (Multidimensional Stiffness Controller)

카르테시안 공간에서 PD 제어:

$$\tau = J^T(\theta)\left(-K_V \dot{x} + K_P \tilde{x}\right) + G(\theta) + F(\dot{\theta}) \tag{27}$$
...
$$M(q)\ddot{q} + n(q,\dot{q}) + J^T(q) K_e(x - x_e) = J^T(q)(-K_V \dot{x} + K_P \tilde{x}) + n(q,\dot{q}) + G(\theta) + F(\dot{\theta})$$

보상 후 정리하면:

$$\boxed{M(q)\ddot{q} = J^T(q)\left(-K_V \dot{x} + K_P \tilde{x} - K_e(x - x_e)\right)} \tag{28}$$
...
$$V = \frac{1}{2}\dot{q}^T M(q) \dot{q} + \frac{1}{2}\tilde{x}^T K_P \tilde{x} + \frac{1}{2}(x - x_e)^T K_e (x - x_e) \tag{29}$$
...
$$\dot{V} = -\dot{q}^T J^T(q) K_V J(q) \dot{q} \tag{30}$$

$$\boxed{\dot{V} \leq 0 \quad \text{(음의 반정부호, negative semi-definite)}}$$

> **해석:** 시스템의 총 에너지는 감소하거나 유지된다.  
> 에너지가 계속 감소 → 시스템은 수렴한다.
...
$$\lim_{t \to \infty} \left[K_P \tilde{x} - K_e(x - x_e)\right] = 0 \tag{31}$$

이를 풀면:

$$\lim_{t \to \infty} x_i = (K_{Pi} + K_{ei})^{-1}(K_{Pi} x_{di} + K_{ei} x_{ei}) \tag{32}$$

### 10.5 정상 상태 결과

#### 정상 상태 힘

$$\lim_{t \to \infty} f_i = K_{ei}(K_{Pi} + K_{ei})^{-1} K_{Pi}(x_{di} - x_{ei}) \tag{33}$$

$K_{ei} \gg K_{Pi}$ 일 때 근사:

$$\boxed{\lim_{t \to \infty} f_i \cong K_{Pi}(x_{di} - x_{ei})} \tag{34}$$

#### 비구속 방향 ($K_{ei} = 0$)

접촉이 없는 자유 공간 방향:

$$\lim_{t \to \infty} x_i = x_{di} \quad \text{(정확한 위치 추종)} \tag{35}$$
...
| 수식 | 설명 |
|------|------|
| $M(q)\ddot{q} + n(q,\dot{q}) + J^T(q)f_e = \tau$ | 매니퓰레이터 동역학 |
| $f_e = K_e(x - x_e)$ | 환경 접촉 모델 |
| $\tau = \tau_t + n(q,\dot{q}) + J^T(q)f_e$ | 중력/동역학 보상 |
| $\tau_t = J^T(q) G_F(f_d - f_e)$ | 직접 힘 제어 |
| $x_d = x_e + f_d / K_e$ | 힘→위치 변환 |
| $f_{ss} \cong K_p(x_d - x_e)$ | 강성 제어 정상상태 힘 |
| $\dot{V} = -\dot{q}^T J^T K_V J \dot{q} \leq 0$ | Lyapunov 안정성 조건 |
